...
def skblur(imarray, user_1, user_2):
    ...
    """
    skblur uses scikit image to apply greyscale to an .jpg image and then apply a Gaussian blur, based on specficied inputs.
    First run "jpg_list" function to provide a convenient list of jpp files to input, the specify arguments.
    After the function has run, it will save an image of size equal to your original image called "blurred_image_YOURFILENAME."
    ...
    user_2: float
        gaussian truncation value, truncates the filter after this many standard deviations 
